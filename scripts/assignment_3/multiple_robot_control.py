...
class robot:
    ...
    def __init__(self, x, y, theta):
        ...
        self.velocities_pub = rospy.Publisher('/' + robotName + '/cmd_vel', Twist, queue_size=10)
        self.pose_sub = rospy.Subscriber(robotName + '/pose', Pose, self.callback)
        #create the generalized q coordinate vector
        self.q = Pose()
        #define the loop frq @ 10Hz
    ...
    def goto_xy(self, x, y):
        ...
        if not self.flag:
            if (atan2(qref.y-self.q.y, qref.x-self.q.x)-self.q.theta)>self.a_tolerance:
...
if __name__ == '__main__':
    ...
    robot3 = robot(8, 4, pi/2) #this robot plots C
